...
最後更新：2026-08-18（Asia/Taipei，T30 日期時區修正已實作完成待部署；CUBE 學費優惠納管、T29 建卡）  
...
## ⚠️ 正式站日期少一天：T30 已修好，**待部署**（2026-08-18）
...
- **T30 已核准並完成實作**：新增 `src/lib/domain-date.ts` 作為全站唯一日期基準，共修改 11 個檔案。
- **自動驗證通過**：`node scripts/verify-t30-date-timezone.mjs`（19 項斷言在 `TZ=UTC` 與
  `TZ=Asia/Taipei` 下結果完全一致）、`tsc --noEmit`、`next lint`。`npm run build` **未執行**。
- **資料未受損**：唯讀盤點 105 個日期欄位，**0 筆偏離**約定，未執行任何資料庫寫入。
- **尚未部署**，因此正式站目前仍然是錯的。人工驗收腳本
  `manual-test-scripts/T30-日期時區修正驗收腳本-v1-2026-08-18.md` **必須在部署後才能執行**。
- **⚠️ 有時限**：`ctbc-uniopen-core-rewards-2026h2`、`ctbc-uniopen-icashpay-stack-2026q3`、
  `ctbc-uniopen-overseas-physical-2026h2` 三筆的 `endDate` 為 **2026/8/31**。
  若未在該日前上線，這三筆會在當天台北上午 8 點錯誤消失。
...
1. **T30 已修好但未部署**（見上方章節）——正式站目前仍在顯示錯誤日期。
   需要使用者授權 push 才能上線，且**建議在 2026-08-31 之前**完成。
