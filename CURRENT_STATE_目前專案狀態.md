...
最後更新：2026-08-21（Asia/Taipei，**T32 Scope A、B、C1 已完成並套用 production**；跨日續前一輪；架構收斂完成並落地：規格書 v9、T29 升 v1.3、T32 完成 Scope A 乾跑；T30 已上線並驗證；兩篇攻略已發布；本機已與正式站分離；**T33 全部完成——dev 與 production 兩個資料庫已對齊、`.env` 已切回 dev、四張 schema 卡解鎖**）
...
## 🔴 本機已與正式站分離（2026-08-20 完成，所有 AI 必讀）

> **本機開發連的是 Neon `dev` 分支，不再是正式站。**
> 完整說明：**`docs/sop/LOCAL_DEV_ENVIRONMENT_本機開發環境與資料庫分支.md`**
> （動任何資料庫之前必讀）

| 動作 | 影響哪裡 |
|---|---|
| `npm run dev`、本機後台 `/admin`、AI 的查詢與寫入、匯入腳本 | **dev 分支** |
| Vercel 上的正式站 | **production 分支**（不受本機影響） |

### ⚠️ 最容易搞混的地方

**在本機後台改的資料，不會出現在正式站。** 2026-08-20 之前不是這樣。

要讓改動真的上線：

- **程式變更** → `commit` + `push`（跟以前完全一樣）
- **資料變更** → 需要另外寫入 `production` 分支（在正式站後台操作，或明確把連線指向 production）

**「我在本機後台改好了」不等於「正式站已經改好了」。**

### 建立過程與現況

- Neon `dev` 分支於 2026-08-20 建立，Parent 為 `production`，**Auto-delete 設為 Never**
- 資料完整複製並驗證：35 優惠／18 卡／9 銀行／2 文章／6 分類／13 張表，與 production 一致
- `.env` 的 `DATABASE_URL` 已由使用者本人改為 dev 分支；原始設定備份於
  `.env.backup-production`（切回用 `cp .env.backup-production .env`）
- **AI 全程未取得也不需要連線字串內容**，驗證以「端點比對＋筆數比對」完成

### 一併查到、先前未記錄的 Neon Free 限制

| 項目 | 行為 |
|---|---|
| **自動休眠** | 閒置後 compute scale to zero。**CLI 首次連線可能直接回 `P1001`，不是資料庫壞了**——先跑查詢喚醒（冷啟動 0.7–1.8 秒）再重試 |
| **History retention 6 小時** | 可做時間點還原，是動 production 時的**最後安全網**（非第一道防線） |
| 分支額度 | 10 個，目前用 2 |
| 儲存 | 0.5 GB，目前用 33 MB（6.7%） |

### `.gitignore` 同步修正

原本只排除 `.env` 與 `.env*.local`，**備份檔如 `.env.backup-production` 不在保護範圍**
（該檔含正式站明碼密碼）。已改為 `.env` ＋ `.env.*` ＋ `!.env.example`。
**這個漏洞先前就存在，本次是第一次被觸發。**

## ▶️ T32 進行中：Scope A、B、C1 已完成（2026-08-21）

任務卡：`docs/implementation/tasks/T32-DATA_ALIGNMENT_資料回歸規格整理.md`（v1.2，**A–F 已核准**）
Summary：`docs/implementation/summaries/T32-DATA_ALIGNMENT_SUMMARY-v1-2026-08-21.md`

**dev 與 production 皆已套用並驗證，兩邊逐項一致。**
...
`/scenarios/hsr-tra` 的變化**要 push 觸發重建才看得到**。

### 三個發現

1. **合併後的目標標籤多數沒有對應頁面**：`捷運公車`(2)、**`海外`(11)**、`首刷禮`(1) 都沒有情境頁，
   只有 `高鐵台鐵` 有。原因是 `捷運公車`／`餐廳`／`超商` 是規格 v8 新增，
   但 `src/lib/domain-scenarios.ts` 的 14 個情境從未加上。
   → **`海外` 11 筆是新增情境頁的最佳候選**（T29 範圍）。
2. **條件標籤不能用關鍵字自動判定**：中信 uniopen 綁 icash Pay 那筆有「需另行登錄」，
   但指的是另一個活動，條文明說「不能直接視為信用卡固有回饋」。純比對會誤掛，
   **掛錯條件標籤比漏掛更糟**。逐筆人工判讀後排除，7 筆變 6 筆。
3. **`需切換` 不在規格 v9 的受控詞彙清單裡**，但早已在使用。規格書應補上（須走專門流程）。

### ⚠️ C1 發現原本的拆分前提不成立

C1／C2 原本依「工作量」拆。執行時逐筆調出我方資料庫的實際文字，
**發現規劃中的 4 筆只有 1 筆（台新 Richart 切換刷）我方資料足以判定**——
其餘三筆的說明只寫「指定通路」「百大指定消費」，沒列出涵蓋哪些通路。

→ **真正的分界不是「哪幾筆」，而是「我方資料夠不夠判定」。**
其餘 3 筆併入 C2，C2 重新定義為 **7 筆，全部需查官網**。

### 尚未執行

順序改為 **E → F → C2 → D**（E、F 不需查官網先做；C2 與 D 都要逐筆查證官方頁）。
**F 的建分類與改 10 筆必須同批**，否則正式站會出現空的分類頁。
...
### ✅ 收尾完成：`.env` 已切回 dev（2026-08-20 23:58）

連線實測 735ms、12 張表、2 筆 migration、35／18／9／2／6 → 確認為 dev 分支。

**兩個方向現在都有備份**（`.env.backup-dev` 與 `.env.backup-production`，皆被 `.gitignore` 排除），
往後切換只需 `cp`，不必再跑 Neon：

| 要切到 | 指令 |
|---|---|
| dev（日常） | `cp .env.backup-dev .env` |
| production（要動正式資料時） | `cp .env.backup-production .env` |

### ⚠️ 切換 `.env` 的兩個坑（本輪各踩一次，已寫進 SOP）

1. **`cp` 會覆蓋，被蓋掉的連線字串就從本機消失**——切換前先確認要被覆蓋的那組有備份。
2. **Neon 連線視窗預設給的是 Default 分支（＝`production`）**，即使從 `dev` 分支頁面點進去也一樣。
   本輪因此**連續兩次**複製到 production 的字串。
   **最危險的是它完全沒有錯誤訊息**：檔案有存、時間戳有更新，但內容逐字元相同。
   → 複製前先把 `Branch` 下拉選單改成 `dev`，並用眼睛確認 `ep-` 那段不是 `bitter-surf`。
   → **驗證要用 `cmp` 逐字元比對 ＋ 實際連線**，不能憑「有存檔」判斷。
