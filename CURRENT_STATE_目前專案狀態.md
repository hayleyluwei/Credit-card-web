...
## ⚠️ Git 授權規則（使用者於 2026-08-06 明訂，對所有 AI 有效）
...
- **上線後的兩項追加修正**：① 手機橫向捲動容器會裁切卡片上緣框線（`overflow-x` 非 visible 時另一軸也變 auto），已加上留白修正；② 首頁 hero 文案依使用者指定改寫為「信用卡回饋大集合！／今天需要哪個優惠？」，英文小字同步換為 `credit card rewards, all in one place`。副作用：首頁最上方原本的「每筆都附官方來源」信任訊息隨舊副標移除，目前只在頁尾。
...
- 正式站是否為最新 commit：**截至 `86d952a`（2026-08-06）為最新已部署 commit；`911eed8` 那一版已實測驗證**（12 個公開 URL 全數 200；表頭／漢堡按鈕／頁尾免責皆存在；舊 `brand-700` 已 0 處；聯絡信箱明碼與 `mailto:` 皆 0 處，代表防爬混淆生效；T26 的 GA script 存在；`/admin/login` 200 且未誤帶公開表頭）。此後若有新 commit push 到 `main`，需重新確認。
...
**使用者於 2026-08-06 外出，約 2026-08-09 後才會再使用電腦，並於離開前開啟新 session。** T28 視覺改版已全部推上正式站並驗證完成，含上線後回報的兩項修正（手機框線裁切、首頁 hero 文案改寫）。

**新 session 請先讀交接摘要**：`docs/implementation/handoffs/2026-08-06-T28設計系統套用與上線-交接摘要.md`
