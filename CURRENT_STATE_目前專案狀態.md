...
最後更新：2026-08-21（Asia/Taipei，**T32 Scope A、B 已完成並套用 production**；跨日續前一輪；架構收斂完成並落地：規格書 v9、T29 升 v1.3、T32 完成 Scope A 乾跑；T30 已上線並驗證；兩篇攻略已發布；本機已與正式站分離；**T33 全部完成——dev 與 production 兩個資料庫已對齊、`.env` 已切回 dev、四張 schema 卡解鎖**）
...
## ▶️ T32 進行中：Scope A、B 已完成（2026-08-21）
...
Scope **C1 → E → F → C2 → D**（F 的建分類與改 10 筆必須同批，否則正式站會出現空的分類頁）。
