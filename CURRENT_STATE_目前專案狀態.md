...
## ⚠️ 寫入資料庫 ≠ 全站可見（2026-08-18 記錄）

部分頁面是 **build 時靜態產生**，資料庫改了不會自動反映，**必須 push 觸發重新部署**：

| 頁面 | 產生方式 | 資料變更後 |
|---|---|---|
| 首頁、`/cards`、`/categories`、`/guides`、`/scenarios/[slug]`（14 頁） | 靜態／SSG | **需重新部署** |
| `/offers/[slug]`、`/categories/[slug]`、`/cards/[slug]`、`/banks/[slug]`、`/search` | 動態 | 即時反映 |

2026-08-18 Codex 新增 CUBE 優惠後只驗證了詳情頁回應 200 就記為「Production 已驗證」，
但當時該優惠**沒有出現在首頁與學費情境頁**——就是踩到這一點。新增或修改資料後務必確認。

## ✅ 正式站日期少一天：T30 已修好並上線（2026-08-18）

日期以台北午夜存入資料庫（例：`2026-06-30T16:00:00Z` ＝ 台北 2026-07-01 00:00，**資料本身正確**），
但 Vercel 以 UTC 執行，`toLocaleDateString` 會輸出早一天的日期。

**2026-08-18 Discovery 後確認為三個相關缺陷**（詳見 `docs/implementation/tasks/T30-DATE_TIMEZONE_日期時區正確性修正.md`）：

1. **顯示早一天**：`toLocaleDateString` 未指定 `timeZone`。`OfferCard` 是 server component，
   所以**首頁、分類、情境、搜尋、卡片頁凡是列出優惠的地方全部受影響**，不只詳情頁。
   `guides` 兩頁用 `toISOString().slice(0,10)`，那是永遠取 UTC，**本機也是錯的**。
2. **過期提早一天**：`setHours(0,0,0,0)` 用伺服器本機時區。實測 `endDate` ＝台北 7/1 的優惠，
   在**台北 7/1 上午 8 點**（UTC 進入 7/1）就被判定過期而消失。
3. **（新發現，最嚴重）後台編輯會讓日期每存一次往前跳一天**：
   `dateInput` 用 `toISOString()` 顯示成前一天，`dateValue` 用無時區的 `new Date("...T00:00:00")` 存回，
   在台北本機儲存一次就往前一天且**會累積**。
   → **無法假設資料庫現有值都還是原始值**；修程式前需先唯讀盤點哪些筆被編輯過。

**根因是專案沒有統一時區約定**：寫入用 `+08:00`，讀取全部用伺服器本機時區。
本機是 Asia/Taipei、Vercel 是 UTC，**所以本機測試永遠看不到這個 bug**。

### 目前狀態（2026-08-18，已解決）

- **T30 已核准、實作、部署並通過上線後驗證**：新增 `src/lib/domain-date.ts` 作為全站唯一
  日期基準，共修改 11 個檔案。commit `d41aed9`，快轉推送 `3dd19a8..d41aed9`（使用者明確授權）。
- **自動驗證通過**：`node scripts/verify-t30-date-timezone.mjs`（19 項斷言在 `TZ=UTC` 與
  `TZ=Asia/Taipei` 下結果完全一致）、`tsc --noEmit`、`next lint`。`npm run build` **未在本機執行**
  （白名單為空，且會寫共用 `.next`），實際把關的是 Vercel 建置。
- **資料未受損**：唯讀盤點 105 個日期欄位，**0 筆偏離**約定，未執行任何資料庫寫入。
- **正式站已驗證**：優惠詳情頁、首頁 OfferCard、搜尋頁、情境頁四種頁型實測日期全部正確
  （例：`2026/12/30` → **`2026/12/31`**）。
- **8/31 的過期誤判風險已排除**（三筆 uniopen 優惠現在顯示正確的 2026/8/31）。
- **待使用者本人逐項驗收**：`manual-test-scripts/T30-日期時區修正驗收腳本-v1-2026-08-18.md`。
  其中 C 區塊（過期判斷）需等到 **2026-08-31 當天**才能驗證。

## 攻略文章：第一篇已建檔，**目前已發布**（2026-08-18）

- **建立了上稿流程 SOP**：`docs/sop/GUIDE_ARTICLE_PUBLISHING_攻略文章上稿流程.md`
  （上稿前檢視清單、上稿與發布步驟、發布後驗證、已知限制）。SOP 索引同時補登原本未列入的撰寫準則。
- **第一篇文章目前已發布**：`slug=tuition`、id=1；2026-08-18 後台列表確認為「已發布」。
  由使用者撰稿，經 AI 依 SOP 審稿後修正並代為建檔（使用者當次明確授權）。
- **審稿發現並修正的重點**：原稿漏了三個關鍵條件——永豐需登錄且限 i 繳費／學費通、
  台新分期需繳納前登錄、台新街口加碼每月限量 55,000 名；並補上台新分期的利率階梯
  （3／6 期 3.88%、12 期 5.88%、18／24／30 期 8.88%）與 CUBE 的資格時間規則
  （每月 20 日前符合、次月 1 日起可切換）與「1% 已含一般消費 0.3%」。
- **slug 刻意設為 `tuition`**，與情境 slug 相同，發布後才會出現在 `/scenarios/tuition` 的延伸閱讀。
- **兩個上稿相關的坑（都已寫進 SOP）**：
  1. **草稿無法預覽**——`/guides/[slug]` 對未發布文章直接 404。已開 **T31** 草案。
  2. **發布文章不會讓情境頁連上它**——`/scenarios/[slug]` 是靜態頁，發布動作沒有重新驗證它，**需重新部署**。

## 攻略文章：中秋聚餐已發布，銀行名稱文案已修正（2026-08-18）

- 使用者先前明確授權 AI 透過正式站後台建立草稿；本次進入後台時文章已為發布狀態。
...
### 內容審查（2026-08-18）：文章數字全部正確，但揪出資料庫缺口

依上稿流程逐項核對後，**文章本身不需要修改**——三次 WebFetch 逐項核對台新官方活動頁，
Chill 刷最高 10%、好饗刷 3.3%、支付工具與排除清單、中秋連假 9/25–9/28（另以政府 115 年
假期公告 PDF 核對）全數逐字相符；DAWHO／CUBE 的數字與我方資料庫一致；9 個連結（3 站內＋6 站外）
全部 200；FAQ JSON 可解析。

真正的問題出在**我方資料庫本身**：文章連到的優惠 `taishin-richart-switch-rewards-2026h2`
原本只記了「最高 3.8%」一個 tier，但官方頁面實際是 LEVEL 1／LEVEL 2 兩層資格 ×8 個子方案
（2%～10%），使讀者從文章點過去會覺得「網站自己打架」。**使用者已授權補齊**，詳見下一節。

## 優惠資料補齊：台新 Richart 卡「切換刷」（2026-08-18，使用者授權）

- 起因：審查中秋聚餐文章時發現的資料庫缺口（見上節）。
- 已將 `taishin-richart-switch-rewards-2026h2` 由 1 筆 tier（最高 3.8%）擴充為 **10 筆**
  （LEVEL 1 基礎 1.3%、Chill刷最高 10%、Pay著刷、天天刷、大筆刷、好饗刷、數趣刷、玩旅刷、
  假日刷、一般消費／保費），內容逐項對應官方原文；`title`／`headlineRate`／`summary`／
  `description`／`tags`／`sourceUrl`／`lastVerifiedAt` 一併更新。
- **寫入前已備份**當時的完整 Offer＋tier 記錄（session scratchpad）。
- **驗證通過**：優惠詳情頁、搜尋頁、分類頁、首頁、sitemap 皆 200；詳情頁正確顯示「最高 10%」
  與「共 10 個回饋層級」；搜尋結果已同步更新，無舊標題殘留。
...
## 資訊架構收斂（2026-08-20，Discovery 完成，決策全數拍板）

起因是 2026-08-18 使用者回報「餐飲美食分類裡沒有外送優惠」。追查後確認不是單筆漏填，
而是資訊架構問題。經一輪「回到使用者角度」的互問互答收斂完成。

**文件：`docs/superpowers/plans/2026-08-20-INFORMATION_ARCHITECTURE_使用者動線與資訊架構收斂.md`**
（後續任務卡的共同依據，新 session 討論分類／標籤／動線前必讀，避免重來一次）

- **10 條使用者動線**（使用者提 7 條、AI 補 3 條），2 條明確不做（快到期提醒、事後查證）。
- **9 項已拍板決策**：Category＝性質大類、Tag＝情境議題、窄議題用攻略文章不新增分類、
  我的卡不記憶不登入、常態權益與限時活動分開呈現、分類頁要分群、滙豐旅遊積分算類現金等。
- **核心架構**：場景（Tag，可多個）× 形式（`rewardType`）兩個獨立軸，分類頁改子題分群。
- **重要結論：第一、二批工作完全不需要改 schema。** 原本以為要動 migration 的
  「常態權益」，經查規格書 §4.4 早已允許無結束日的 Offer，且前台程式碼完全支援——
  是資料合規問題，不是架構缺陷。
- **⚠️ 與資料蒐集規格書有實質衝突**：§4.2「每筆優惠必須歸入恰好一個主分類」與雙軸架構
  牴觸，需出 v3；否則新資料會依舊規則建檔，破壞整理成果。
- **⚠️ T29 核准前需先確定場景標籤最終清單**，否則 Scope D 的「14 個情境標籤」前提會過時。
- **共同阻擋點**：獨立開發資料庫（T27 待決 (g)，自 2026-08-05 未解）擋住所有需要
  migration 的工作（T27、T29、大額試算）。
### 2026-08-20 已執行（架構決策落地第一步）

- **資料蒐集規格書出 `v8-2026-08-20`**（v7 已標記被取代）：分類改為場景軸（5，含新增
  `bill-payment` 帳單繳費）與形式軸（2）、主分類角色改變（分類頁改依標籤收錄）、
  新增標籤受控詞彙與同義詞禁用表、情境標籤 14→17 項、回饋類型五選一→七選一
  （點數拆成等值／非等值，新增餐券贈品）、強化長期權益填寫要求、品質驗收新增 3 項。
- **T29 升版 v1.3**：Scope D 改為 D15 動態規則，前置條件解除；Scope B 上調為核心。
- **新增 T32 資料回歸規格整理（草案待核准，v1.1）**：A–F 六項，**全部寫入正式站資料庫**。
  **Scope A 已完成唯讀乾跑模擬**：影響 12/35 筆，**沒有任何優惠會從情境頁消失**，
  只有「高鐵台鐵」+1、「捷運公車」+2。最具代表性的一筆（中信 LINE Pay 一卡通）
  自己就掛了 5 個同義詞，而 5 個都連不到任何頁面。
- **規格書再出 `v9-2026-08-20`（同日修正 v8）**：乾跑發現 v8 兩條受控詞彙規則會造成
  資訊損失——① `新戶`（身分）不該吸收 `首刷禮`（優惠類型），改為兩組分開；
  ② `旅遊` 不該併入 `旅遊訂房`（4 筆實為海外消費／哩程／機場禮遇／機票），改為直接移除。
  另依使用者指示新增**長期權益至少每半年複查一次**的規則（§4.4、品質驗收第 13 項）。
- **更正**：架構文件 v1 誤植「規格書最新版為 v2」，實際為 v7；已於 v1.2 更正。
  同時補記 v7 §4.3 早有「切換方案要補齊所有情境標籤」規則但資料未遵守。

**貫穿本輪的模式：規格書走在前面，資料沒跟上。** 長期權益（v1 起就有規則、0 筆遵守）、
切換方案標籤（v7 起就有、3 筆全沒補）、標籤同義詞（v2 起就禁、仍在漂移）皆然。
所以 T32 的性質是「讓資料回到規格」，不是「改規格」。

- **6 個待決問題已於 2026-08-20 全數拍板**（D10–D15）：文章移到首頁前面但**等累積 5 篇再動**；
  空子題顯示「暫無優惠」；`交通通勤` 與 `旅遊交通` **保留不合併**；現金回饋／分期零利率改為
  **形式軸主題頁**；**新增「帳單繳費」大類**（可收 10 筆，目前全散在 cashback）；
  **標籤可否點擊改為動態規則**（有內容才可點，取代原本 14 個白名單）。
- **⚠️ T29 任務卡需改寫升版**：Scope D 的白名單規則已被 D15 取代；Scope B（文章導回優惠）
  由次要升為核心。原本「T29 核准前須先確定 14 個標籤清單」的前置條件**已解除**。

## 全專案一致性盤點結果（2026-08-20）

使用者要求「全部檢視整個專案的規格書、schema、任務卡」後執行，逐項實際查證：

- 🔴 **產品規格書落後現實兩個半月**：`credit-card-mvp-spec-v11-2026-06-08.docx` 是最新版
  （Word＋PDF 配對完整）。全文抽出比對確認**完全沒有** `/guides`、`/scenarios`、
  `RewardTier`、卡片生活誌設計系統、GA、長期權益——那是六月的產品。
  **使用者已核准出 v12**，但依 `規格書修改流程.md` 第 3、4 條**必須先列修改清單取得確認**，
  清單尚未產出。
- 🔴 **正式資料庫有 `playing_with_neon`**：Neon 建專案時的範例表（`id`／`name`／`value`，
  10 筆假資料）。**專案零引用**，但會讓將來 `prisma migrate` 判定 drift 並建議重置資料庫。
- 🟡 **Neon Free 會自動休眠**：`prisma migrate diff` 第一次連線直接失敗，
  Prisma 查詢喚醒需 673ms。**CLI 首次連線失敗不代表資料庫壞了。**
- 🟢 **schema 三份副本完全同步**，我方 11 張表與 schema 檔**零漂移**。
  schema 專門流程是本專案治理做得最好的部分。
- **任務卡衝突 5 項**（尚未寫進卡片，記錄在交接摘要）：T27 描述的版位預留現況已被
  T28 v2.1 取代、長期權益讓 T25 前提破一半、新戶禮比較需要 T24 才完整、
  四張卡全卡在 schema 變更、三處事實過時。

## ▶️ 下一步：建立 Neon dev branch（使用者已指定）

**現況**：`prisma/schema.prisma` 自己寫著 "also used for local dev"——本機開發直連正式站。
本輪所有資料寫入都是直接寫線上。

**要做**：Neon 建 `dev` 分支（Free plan 有 10 個）→ 本機 `.env` 指向它 →
在 dev 分支上處理 `playing_with_neon` 與 migration 對齊。

**⚠️ 連線字串含明碼密碼，使用者自己貼進 `.env`，不要貼進對話或任何文件。**

**⚠️ 改 `.env` 前必須先確認 Vercel 環境變數是獨立設定的**，確保正式站不受影響。

**解鎖效果**：T24／T25／T27／T29 四張都要改 schema，目前動就是直接改正式站。
...
背景：T28 上線期間共推送三次，其中兩次（文件同步、手機框線修正）是 AI 自行判斷「既有授權延伸」而推的。使用者事後追問並訂下此規則。

**授權是單次、單一情境的，不會自動延伸。** 下列情況同樣要先問：

- 修正「剛推上去的東西造成的問題」
- 純文件、未動 `src/` 的變更
- 使用者即將離線或看起來趕時間

local commit 不受此限，仍依各任務卡的 Git 授權進行。
...
5. **最終資料量**：9 家銀行／18 張卡／36 筆優惠列（**34 個優惠代號**，共 36 層 RewardTier，含 1 筆 3 層優惠）／45 筆優惠卡片對應。
   - **2026-08-05 更正**：本行原記為「26 個優惠代號」，經 T22 開工時直接查詢正式站資料庫核對，實際為 **34 筆 Offer**（34 筆中 1 筆為 3 層、33 筆單層＝36 層 RewardTier，數字自洽）。其餘數字（9 銀行／18 卡／36 tier／45 對應）皆與實際相符，僅此一項為筆誤。34 筆全部 `isPublished=true` 且無過期。
...
### T22 排程輔助資料更新 — **Scope v2 瘦身版已核准，端對端驗收通過，長期營運中**（2026-08-06）
...
#### 端對端驗收結果（2026-08-06，已完成）

使用者本人完成三項前置動作並設定 GitHub Secrets：`DATABASE_URL_READONLY`（Neon 唯讀角色 `offer_checker_ro`，經三段查詢驗證僅有 `SELECT` 且無繼承群組，涵蓋 13 張表；過程中曾用較弱的臨時密碼、經回報後已用 `ALTER ROLE` 立即輪替）、`TELEGRAM_BOT_TOKEN`、`TELEGRAM_CHAT_ID`。

手動觸發 GitHub Actions（run `31065358168`）：**前四步全數成功，「執行來源檢查」以唯讀憑證成功連線並跑完全部 34 筆（32 成功／2 失敗／2 疑似有變，與既有查證的 4 筆一致，非新問題）；`Telegram 通知已送出。`，使用者已收到內容一致的通知，確認端對端貫通。** Job 以 `exit code 1` 結束、Actions 顯示紅燈——此為設計行為（有失敗筆數即回傳非零狀態碼），非執行錯誤。

至此「驗證方式與完成定義」四項驗收全數達成，任務轉為**每月 1 日自動執行的長期營運狀態**，非阻塞待辦。

#### 尚未完成（非阻塞，皆會改變系統行為，待使用者決定是否進行）

- **4 項改善候選**：區分「被銀行擋下」與「頁面異常」（`ctbc-uniopen` 兩筆已知會穩定重現）、來源網址粒度偏總覽（`taishin-richart` 一例）、費率置於圖片的來源宜標記為不適用自動檢查、疑似暫時性差異（`ctbc-uniopen-brand-weekly-promos` 兩輪結果不同）是否加重試機制。
- 本機測試時 Playwright 曾被執行環境沙箱擋住，改以停用沙箱方式跑通（操作本身為唯讀抓取公開網頁）；GitHub Actions 雲端環境無此限制，已於雲端成功驗證。
...
- 頁尾聯絡信箱 `hayleylushop@gmail.com` 已做防爬混淆（base64＋掛載後才解出，伺服器輸出的 HTML 不含明碼）。**注意此信箱與管理員登入帳號 `hayleylu0902@gmail.com` 不同，是刻意分開的對外聯絡信箱。**
- **上線後的兩項追加修正（2026-08-09）**：① 手機橫向捲動容器會裁切卡片上緣框線（`overflow-x` 非 visible 時另一軸也變 auto），已加上留白修正；② 首頁 hero 文案依使用者指定改寫為「信用卡回饋大集合！／今天需要哪個優惠？」，英文小字同步換為 `credit card rewards, all in one place`。副作用：首頁最上方原本的「每筆都附官方來源」信任訊息隨舊副標移除，目前只在頁尾。
- **管理員密碼**：使用者於 2026-08-06 表示忘記。已建立 `scripts/reset-admin-password.mjs`（預設 dry-run，需 `--confirm`；密碼在執行者本機以 `node:crypto` 產生、只印一次、不寫檔、AI 不接收）。**使用者尚未執行**，下次要用後台需自行重設。

### T29 情境／攻略／優惠三方串接 — v1.2 草案待核准（2026-08-18 建卡）

- 任務卡：`docs/implementation/tasks/T29-CONTENT_CROSSLINKS_情境攻略優惠三方串接.md`
- **起因**：使用者檢視 T28 上線後的首頁情境卡，指出 T28 把標題改成問句（「繳稅金額大，回饋能省下多少？」）之後，**卡片承諾了答案，但點進去只有一份優惠清單**——清單回答不了「能省多少」「划算嗎」，那是攻略文章的守備範圍。「帳單日常」那一組（繳稅／學費／水電／保費）特別明顯。
- **使用者同時要求反方向**：攻略文章讀完要能導回具體優惠——回到該情境的優惠列表，或直接導到特定卡片的優惠頁。目前文章是內容終點、不是行動起點。
- **既有基礎不需重做**：T20 已有文章功能與「slug 相同即視為對應攻略文」的關聯機制；T28 已有 `SCENARIO_COPY` 集中管理情境卡文案。
- **關鍵前提**：目前 **0 篇已發布文章**，因此任何連結改動在有文章之前都沒有可見效果。待決問題 (e) 建議先寫第一篇（「繳稅」——問句最具體、現有優惠資料最多）再做串接，否則無從驗證。
- **(b) 已拍板**：文章導回優惠採「自動依標籤撈為主、手動明確指定優先」；連帶確定會新增 `Article` 選填欄位＝**改 schema，須走 schema 專門流程**。
- **2026-08-18 併入 Scope D**：使用者查繳稅優惠時發現 `taishin-jkopay-card-bill-payments-2026` 點進去只顯示「現金回饋」。實測確認**資料無誤**（確實掛了繳稅標籤，情境頁與搜尋都找得到），真正缺口在**優惠頁不顯示標籤、也沒有連回情境頁的入口**——標籤只存在於 JSON-LD，人看不到。做法為優惠頁新增一列可點的情境標籤；只有 14 個核心情境可點，其餘標籤維持灰色不可點以免產生死連結。
- 其餘 7 個待決問題未拍板。未核准前不得修改任何前台頁面或資料。
...
- 正式站是否為最新 commit：**截至 `7637dcb`（2026-08-09）為最新已部署 commit**；`911eed8`（2026-08-06 首次部署）與 `86d952a`（2026-08-09 文案改寫）兩版皆已實測驗證（12 個公開 URL 全數 200；表頭／漢堡按鈕／頁尾免責皆存在；舊 `brand-700` 已 0 處；聯絡信箱明碼與 `mailto:` 皆 0 處，代表防爬混淆生效；T26 的 GA script 存在；`/admin/login` 200 且未誤帶公開表頭）。此後若有新 commit push 到 `main`，需重新確認。
...
**2026-08-09 更新**：使用者 2026-08-06 外出後已於 2026-08-09 返回並繼續本輪工作。T28 視覺改版已全部推上正式站並驗證完成，含上線後回報的兩項修正（手機框線裁切、首頁 hero 文案改寫）。

**新 session 請先讀交接摘要**：`docs/implementation/handoffs/2026-08-18-CUBE優惠納管與T29建卡-交接摘要.md`
...
1. ~~正式站日期少一天~~ **已於 2026-08-18 修好並上線**（T30，見上方章節）。
   剩下使用者本人的逐項驗收，以及 8/31 當天的過期判斷確認。
2. **T28 人工驗收**（後台密碼已重設，E 區塊現在跑得了）：`manual-test-scripts/T28-設計系統套用驗收腳本-v1-2026-08-06.md`
3. **T29（情境／攻略／優惠三方串接）已建卡待核准**。其中 Scope D（優惠頁可點情境標籤）**不依賴文章、可獨立生效**；Scope A、B 則需要先有攻略文章才看得到效果，建議先寫第一篇（「繳稅」）。
4. T26 人工驗收：登入 GA4 看「即時」報表確認有流量。

**資料面現況**：Codex 已新增 CUBE「童樂匯指定私校學費 1%」，經 Claude 逐項複核官網（10 項吻合）、補上第二個來源網址並完成增量匯入（更新 1 筆，總數維持 35）。全部已納入版控。
...
4. T22（排程輔助資料更新）**已於 2026-08-05 核准 Scope v2 瘦身版，2026-08-06 完成端對端驗收**（三項 GitHub Secrets 設定完成、GitHub Actions 手動觸發成功跑完 34 筆、Telegram 通知確認送達）。**已轉為每月 1 日自動執行的長期營運狀態，非阻塞待辦。** 詳見上方 T22 段落。
