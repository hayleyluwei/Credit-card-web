...
DOCX_PATH = OUT_DIR / "credit-card-mvp-spec-v8-2026-06-05.docx"
TEXT_DUMP = OUT_DIR / "credit-card-mvp-spec-v8-2026-06-05-inspection.txt"
...
p2 = doc.add_paragraph()
style(p2.add_run("版本：2026-06-05 / v8\n本版在 v7 基礎上將第 9 節改為開發期 / 上線期分階段版本，並補充 cards.image_url 的圖片存放策略，讓本機開發與正式上雲路線更清楚。"), 11, False, "394651")
...
grid_table(doc,
    ...
    [
        ...
        ("cards", "bank_id / name / slug / image_url / summary / is_active", "在信用卡管理建立卡片基本資訊，供列表頁卡名、銀行、卡面圖與詳情頁資料引用；image_url 儲存卡面圖的最終公開 URL。"),
...
kv_table(doc, [
    ...
    ("cards.image_url 圖片策略", "開發期可先使用本機或專案內可控圖片路徑作為測試資料來源；正式上線時，卡面圖應存放於託管物件儲存或雲端媒體服務，資料庫中的 image_url 只保存最終可公開存取的圖片網址，不直接保存圖片檔本體。"),
])
...
heading(doc, "9. 技術建議與開發順序", "以開發速度、前期成本與正式上線可行性平衡，建議將技術規劃分成開發期與上線期兩個階段。")
grid_table(doc,
    ["階段", "建議方案", "目的與說明"],
    [
        ("開發期", "Next.js / Prisma / SQLite / Tailwind CSS", "先在本機完成前台、後台、CRUD、排序、摘要與 SEO 欄位驗證。SQLite 適合前期快速開發與測試資料，不需先建立雲端資料庫。"),
        ("上線期", "Next.js 部署至 Vercel 或同類型 PaaS；資料庫改為託管 PostgreSQL；持續使用 Prisma", "正式對外服務時，不使用本機 SQLite 作為正式資料庫。前台與後台部署到託管平台，資料庫改為雲端託管服務，讓內容提供者無需自建伺服器與資料庫。"),
    ],
    widths=[0.9, 2.2, 3.3],
)
numbered(doc, [
    "開發期先完成資料庫 schema、後台登入與後台 CRUD，再串接前台首頁、分類頁與詳情頁。",
    "開發期完成測試資料後，進行 RWD 與閱讀節奏微調，確保桌機與手機都好讀。",
    "準備上線時，將 Prisma datasource 從 SQLite 切換為託管 PostgreSQL，並部署前台與後台到 Vercel 或同類型雲端平台。",
])
...
heading(doc, "11. 本版更新說明", "以下為本版（2026-06-05 / v8）相較上一版 credit-card-mvp-spec-v7-2026-06-05 的新增、修正與補強內容。")
kv_table(doc, [
    ("新增", "新增開發期 / 上線期分階段技術規劃，明確區分本機開發使用 SQLite 與正式上線改用託管 PostgreSQL 的路線。"),
    ("修正", "補充 cards.image_url 的圖片存放策略，說明開發期可用本機測試路徑，正式上線時改用託管物件儲存或雲端媒體服務，資料庫只保存公開 URL。"),
    ("補強", "補強內容提供者模式下的正式上雲說明，讓閱讀者可理解網站不會只停留在本機，而是可部署到 Vercel 或同類型 PaaS。"),
    ("版本處理", "新版檔案另存為 credit-card-mvp-spec-v8-2026-06-05.docx，未覆蓋原本的 v7。"),
])
