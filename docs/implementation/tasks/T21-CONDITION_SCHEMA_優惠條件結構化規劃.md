...
> **狀態說明**：本任務卡只記錄規劃討論結果，**不構成任何實作授權**。Scope v1 描述的是「若核准後預計執行的工作」，在使用者針對「風險與待決問題」逐項拍板、並正式核准 Scope 版本之前，不得修改 Prisma schema、不得建立 migration、不得變動任何程式碼。
...
- 撰寫資料遷移腳本：把既有 20+ 筆 seed offers 與 2 筆 DAWHO 正式資料的扁平回饋欄位，轉換成至少一筆 `RewardTier`，並驗證遷移前後前台顯示內容一致。
