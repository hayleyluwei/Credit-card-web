...
## 上線後驗收回饋修正（Scope v2.1，2026-08-18）

使用者於 2026-08-18 檢視上線成果後提出四項回饋（以 pptx 標註交付）。經確認**屬於 T28 自身產出的修正，
不另開任務卡**——理由是 T28 的人工驗收本來就尚未執行，這些正是驗收該產出的東西；且改的是 T28 寫的文案，
不是新功能。另開新卡會讓 T28 永遠停在「待驗收」，而修正散落他處。

### 一、情境選讀版位改為正式文案（使用者選擇「整塊改寫」）

原本這個版位是 T27 的視覺預留，文案明講「版位預留中」。使用者指定改寫標題與大標後，
若只改那兩句，區塊會變成「行銷語氣的大標」與「施工中的告示」並存，語氣自相矛盾，
因此經使用者拍板**整塊改為正式文案**：

| 位置 | 修正前 | 修正後 |
|---|---|---|
| 區塊標題 | 本月情境選讀 | **情境選讀** |
| 區塊副標 | 每個月挑一件你可能正在煩惱的事（版位預留中） | **精打細算，玩樂更歡** |
| 黑底小標 | 版位示意 | **生活提案** |
| 黑底大標 | 每個月，我們挑一件你可能正在煩惱的事。 | **生活中有大大小小與消費相關的事，善用信用卡，聰明消費。** |
| 黑底內文 | 這個位置的樣式已經定案，實際內容要等後台可以按月指定選讀之後才會填入⋯⋯ | **繳稅、學費、加油、訂閱⋯⋯每一種花費都有適合的卡。先挑一個你最近會遇到的情境開始看。** |
| 右側白卡 | 待後台支援／本月選讀內容／連向真實存在的情境或攻略頁，不會是死連結。 | **這個月／旅遊訂房／訂房、機票和海外消費常常分屬不同活動，出國前先看懂哪張卡划算。** |
| 行動按鈕 | 先看旅遊情境 →（真連結） | 不變 |

**與 T27 的關係**：推薦目標目前寫死在 `src/app/page.tsx`。T27 完成後改為後台按月指定、由資料驅動。
版位與視覺仍由 T28 負責，此次只換文案與寫死的推薦內容，未動版型。

### 二、熱門情境區塊標題

「你最近想完成什麼？」→ **「生活中的消費有沒有回饋」**（使用者指定）。

### 三、情境卡標題標示重點字

使用者回報「標題字太多，第一眼認不出這張卡在講什麼優惠」，要求把「繳稅」「學費」等關鍵字標明顯，
並指定**須在風格契約允許範圍內**處理。

契約查核結果決定了作法：

- `blue`／`blue-deep` 在契約中是「主要行動 hover／連結」專用色，**拿來標關鍵字會與連結語意衝突**，不可用。
- 標題字重已是契約規定的上限 `850`（§3），往上沒有空間；標題內混不同字重也會破壞「中文不使用負字距、
  字重分層固定」的規則。
- `lime` 在契約 token 表中的用途即「萊姆亮點」（§2）。**以底色標示不動字重、字距、圓角與框線**，
  是唯一完全落在契約內的作法。

實作：`ScenarioCopy` 新增選填欄位 `highlight`，並新增 `splitScenarioTitle()`。
14 個情境中有 10 個標題本身就含情境標籤，自動沿用；其餘 4 個明確指定關鍵字
（`travel-booking`→出國、`supermarket`→採買、`subscription`→訂閱、`roadside-assistance`→車子拋錨）。
標題找不到關鍵字時整段原樣輸出，不做標示——寧可不標，也不標錯位置。

### 四、分類卡淡藍底色：**確認不修改**

使用者回報 `/categories` 六張分類卡輪替底色中的「淡藍」在桌機幾乎看不出是藍色，手機才看得出來，
並詢問是否為個人螢幕因素。實測數據：

| 色票 | 相對亮度 | 彩度 |
|---|---|---|
| `blue-soft` `#E9F0FF` | 86.9% | **8.6%** |
| `lime` `#DFFF6B` | 88.3% | 58% |
| `yellow` `#FFF1A8` | 87.0% | 34% |
| `rose` `#FFDDE3` | 78.5% | 13.3% |

亮度不是原因（淡藍與萊姆幾乎一樣亮，對白卡的對比甚至略高）。原因是**彩度只有 8.6%，
是四個色塊中最低、萊姆的七分之一**：它靠色相而非明暗與白色區分，這點彩度在桌機 IPS
面板與一般亮度下容易被吃掉，手機螢幕彩度高且觀看距離近就看得出來。

即：使用者看到的現象為真，但屬色票本身的設計取捨，非缺陷。`blue-soft` 為契約鎖定 token，
修改需走風格契約變更流程。**使用者明確表示不改色票，本項不做任何修改。**

### 修改檔案

| 檔案 | 修改 |
|---|---|
| `src/app/page.tsx` | 情境選讀版位整塊文案、熱門情境標題、情境卡標題改為分段渲染並標示關鍵字 |
| `src/lib/domain-scenario-copy.ts` | `ScenarioCopy` 新增 `highlight`；新增 `splitScenarioTitle()`；4 個情境補上關鍵字 |
| `manual-test-scripts/T28-設計系統套用驗收腳本-v1-2026-08-06.md` | B6 改寫（原本要求「清楚說明是版位預留」，已與新文案牴觸），新增 B8、B9 |

### 驗證

- `npx tsc --noEmit --incremental false`：通過。
- `npx next lint`：通過。
- **視覺未經瀏覽器實測**：未啟動 dev server（會寫共用 `.next`，且未列入驗證白名單）。
  萊姆底色標示的實際呈現待人工驗收或使用者授權後以 dev server 確認。
- **未部署**。
...
- v2.1／2026-08-18：上線後驗收回饋修正。情境選讀版位由「版位預留」文案改為正式文案（整塊改寫，使用者拍板）、熱門情境區塊改名、情境卡標題以 `lime` 底色標示關鍵字（契約內唯一可行作法）。分類卡淡藍底色經實測確認為色票彩度取捨而非缺陷，使用者決定不修改。詳見「上線後驗收回饋修正」章節。
