...
最後更新：2026-08-18（Asia/Taipei，T30 已核准並完成實作，待部署後人工驗收；更正 T29 版本為 v1.2）  
...
| T29 | 情境／攻略／優惠三方串接 | v1.2 草案，待核准；**(b) 已拍板為「自動依標籤撈為主、手動明確指定優先」**，連帶確定會新增 `Article` 選填欄位＝改 schema；其餘 7 個待決問題未拍板（含 Scope D 的標籤列位置） | 規劃中（待核准） | 不適用 | T20（文章功能與 slug 對應機制已存在）、T28（情境卡文案與 `SCENARIO_COPY` 已建立）；與 T27 的導向規則需一致 | `tasks/T29-CONTENT_CROSSLINKS_情境攻略優惠三方串接.md` | 無 |
| T30 | 日期時區正確性修正（正式站日期少一天） | **v1 已核准（2026-08-18）**；7 個待決問題中 6 個已解決，僅「何時部署」待授權 | **自動驗證通過，待人工驗收**：新增 `src/lib/domain-date.ts` 作為全站唯一日期基準，修正三個缺陷（顯示早一天／過期提早一天／後台編輯累積往前跳），共改 11 個檔案；Scope E 唯讀盤點確認 105 個日期欄位全數完好、**不需修資料**。人工驗收**必須先部署**才有意義 | **未部署**。⚠️ 有時限：三筆 uniopen 優惠會在 **2026-08-31** 觸發過期誤判 | 無（獨立 Bug 修正；與 T25 過期轉址有概念關聯但不互相阻塞） | `tasks/T30-DATE_TIMEZONE_日期時區正確性修正.md` | 無（以任務卡「實作結果」章節代替） |
