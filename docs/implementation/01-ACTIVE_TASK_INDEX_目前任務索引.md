...
| T22 | 排程輔助資料更新（瘦身版：抓取＋比對＋Telegram 通知；完整版列為未來階段） | **v2 已核准（2026-08-05）**；(a)–(g) 七題皆已拍板，其中 (e)(f)(g) 歸屬未來完整版 | **待人工驗收**：第一輪實作完成並 commit（`e37f07f`、`5fb5a88`）；34 筆全量實測 30 無變動／2 疑似有變／2 健康檢查未過，4 筆皆查證無資料錯誤；60 項離線單元測試全過 | 不適用（獨立腳本，不影響前台） | T18、T21 皆已完成；**端對端驗收待使用者完成前置動作**：Telegram Bot Token、Neon 唯讀憑證、GitHub Secrets（`DATABASE_URL_READONLY`／`TELEGRAM_BOT_TOKEN`／`TELEGRAM_CHAT_ID`） | `tasks/T22-SCHEDULED_DATA_UPDATE_排程輔助資料更新.md` | 無（待驗收後補） |
