...
最後更新：2026-08-21（Asia/Taipei，T33 完成；T29 升 v1.4；T32 核准 A–F 並完成 Scope A、B、C1）  
...
| T32 | 資料回歸規格整理（第一批） | **v1.2，Scope A–F 已核准（2026-08-21）**；四項執行決定由使用者授權 AI 拍板（Richart 補回方式、F 的 10 筆分類須同批、D 先做 1 張、uniopen 拆 C2）。順序 A→B→C1→E→F→C2→D | **Scope A、B、C1 已完成**（dev 與 production 皆已套用並驗證）：15 個禁用詞由 28 次出現清為 0、`需登錄` 9→11、`需切換` 1→5、Richart 切換刷補 5 個情境標籤；**`高鐵台鐵` 0→1、`訂閱服務` 0→1、`旅遊訂房` 6→7、`加油` 2→3**，其餘 10 個情境頁不變，資料未受損。**C1 發現原拆分前提不成立**（4 筆只有 1 筆我方資料足以判定），其餘 3 筆併入 C2；順序改為 **E→F→C2→D** | A、B **已部署並驗證**（`/scenarios/hsr-tra` 正式站確認收錄 1 筆）；**C1 資料已寫入 production，靜態頁待重新部署**；人工驗收未執行 | 規格書 v9-2026-08-20；架構文件 2026-08-20 | `tasks/T32-DATA_ALIGNMENT_資料回歸規格整理.md` | `summaries/T32-DATA_ALIGNMENT_SUMMARY-v1-2026-08-21.md` |
| T33 | 資料庫遷移基準與範例表清理 | **v1 已核准（2026-08-20）**；(b)(e) 已拍板（不備份假資料、不改 `db:push`），另新增決策 7「不得把 SQL 當使用者把關點」 | **自動驗證通過（A–F 全部完成）**：dev 與 production **兩個分支皆已套用** migration `20260820230418_drop_playing_with_neon`，表數 13→12、`_prisma_migrations` 1→2 筆、漂移歸零、資料筆數零變動（35／18／9／2／6）、本機與正式站前台皆正常。**E3 由使用者本人執行**（AI 被 auto mode 擋下，已寫成 SOP 常規）。**收尾完成**：`.env` 已切回 dev 並實測連線，兩個方向的備份皆已建立 | 不適用（網站零引用該表，外觀與行為不變，**不需部署**） | Neon `dev` 分支（2026-08-20 已建立）；**T24／T25／T27／T29 四張卡的前置，現已解鎖** | `tasks/T33-DB_MIGRATION_BASELINE_資料庫遷移基準與範例表清理.md` | `summaries/T33-DB_MIGRATION_BASELINE_SUMMARY-v1-2026-08-20.md` |
