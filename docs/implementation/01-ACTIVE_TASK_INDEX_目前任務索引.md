...
| T22 | 排程輔助資料更新（瘦身版：抓取＋比對＋Telegram 通知；完整版列為未來階段） | **v2 已核准（2026-08-05）**；(a)–(g) 七題皆已拍板，其中 (e)(f)(g) 歸屬未來完整版 | **實作中**（Git 授權：add＋local commit，不含 push；第三層一次授權 Playwright／GitHub Actions／Telegram 三項） | 不適用 | T18、T21 皆已完成；**待使用者完成前置動作**：Telegram Bot Token、Neon 唯讀憑證、GitHub Secrets 設定 | `tasks/T22-SCHEDULED_DATA_UPDATE_排程輔助資料更新.md` | 無 |
