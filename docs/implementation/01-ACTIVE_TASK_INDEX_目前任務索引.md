...
| T32 | 資料回歸規格整理（第一批） | **v1.2，Scope A–F 已核准（2026-08-21）**；四項執行決定由使用者授權 AI 拍板（Richart 補回方式、F 的 10 筆分類須同批、D 先做 1 張、uniopen 拆 C2）。順序 A→B→C1→E→F→C2→D | **Scope A、B 已完成**（dev 與 production 皆已套用並驗證）：15 個禁用詞由 28 次出現清為 0、`高鐵台鐵` 情境頁 0→1、其餘 13 個情境頁不變、`需登錄` 9→11、`需切換` 1→5、資料未受損。**C1／C2／D／E／F 尚未執行** | **資料已寫入 production；靜態頁待重新部署**（`/scenarios/hsr-tra` 的變化需 push 觸發重建才看得到）；人工驗收未執行 | 規格書 v9-2026-08-20；架構文件 2026-08-20 | `tasks/T32-DATA_ALIGNMENT_資料回歸規格整理.md` | `summaries/T32-DATA_ALIGNMENT_SUMMARY-v1-2026-08-21.md` |
