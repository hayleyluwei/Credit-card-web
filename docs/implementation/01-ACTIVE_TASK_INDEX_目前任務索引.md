...
最後更新：2026-08-18（Asia/Taipei，新增 T29 草案；CUBE 學費優惠已納管）  
...
| T29 | 情境／攻略／優惠三方串接 | v1.1 草案，待核准；**(b) 已拍板為「自動依標籤撈為主、手動明確指定優先」**，連帶確定會新增 `Article` 選填欄位＝改 schema；其餘 7 個待決問題未拍板（含 Scope D 的標籤列位置） | 規劃中（待核准） | 不適用 | T20（文章功能與 slug 對應機制已存在）、T28（情境卡文案與 `SCENARIO_COPY` 已建立）；與 T27 的導向規則需一致 | `tasks/T29-CONTENT_CROSSLINKS_情境攻略優惠三方串接.md` | 無 |
